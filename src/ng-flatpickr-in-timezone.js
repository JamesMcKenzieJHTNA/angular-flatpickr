  ...
  ngFlatpickrInTimezone.directive('ngFlatpickr', function($rootScope, $parse, $q) {
      return {
          ...
          link : (scope, element, attrs) => {
              let fpOpts = $parse(attrs.fpOpts)(scope),
                  flatPickrl10n = $q.defer();
               ...
               * @returns {String}
              */
              fpOpts.formatDate = (format, dateObj) => {
                  let chars = format.split('');
                  let flatpickrFormats = Flatpickr.prototype.formats;

                  // Adapted from flatpickr source
                  let dateString = chars.map(function(c, i) {
                  ...
                  if (fpOpts.timezone) {
                      let timezone = fpOpts.timezone || 'UTC';
                      if (format === fpOpts.dateFormat) {
                          ...
                          let utcMoment = moment.utc(dateObj)
                          ...
                          let utcMoment = moment.utc(dateObj) // Convert the dateObj to UTC time
                                          // Add the browser's utcOffset in minutes
                                          .add(moment().utcOffset(), 'minutes');

                          let timezoneAbbr = utcMoment
              ...
              let flatPickr = new Flatpickr(element[0], fpOpts);
              return flatPickr;
          }
      };
  });
